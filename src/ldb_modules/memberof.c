...
static int mbof_mod_process(struct mbof_mod_ctx *mod_ctx, bool *done)
{
     ...
     *   - no ghost changes
     *   - above threshold
     *   - no added member is a group (checked via objectCategory —
     *     the batch path does not handle transitive memberOf
     *     propagation to nested group members)
     */
    total = 0;
    ...
    if (priv && priv->batch_enabled
        ...
        for (k = 0; k < mod_ctx->mb_add->num; k++) {
            struct ldb_result *mbr_res = NULL;
            int sr;

            sr = ldb_search(ldb, mod_ctx, &mbr_res,
                            mod_ctx->mb_add->dns[k], LDB_SCOPE_BASE,
                            NULL, NULL);
            if (sr == LDB_SUCCESS && mbr_res->count == 1
                && entry_is_group_object(mbr_res->msgs[0]) == LDB_SUCCESS) {
