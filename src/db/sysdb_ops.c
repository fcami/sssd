                              ...
                              gid_t gid)
{
     ...
     * Collect usernames for a single batched memberuid modify.
     */
    {
        struct ldb_message *uid_msg = NULL;
        struct ldb_message_element *uid_el = NULL;
        const char *group_dn_str = ldb_dn_get_linearized(group_dn);
        int added_count = 0;

        uid_msg = ldb_msg_new(tmp_ctx);
        if (uid_msg == NULL) {
            ret = ENOMEM;
            goto done;
        }
        uid_msg->dn = group_dn;
        ret = ldb_msg_add_empty(uid_msg, SYSDB_MEMBERUID,
                                LDB_FLAG_MOD_ADD, &uid_el);
        if (ret != LDB_SUCCESS) {
            ret = sysdb_error_to_errno(ret);
            goto done;
        }
        ...
        for (i = 0; i < (int)new_members_el->num_values; i++) {
            ...
            ret = ldb_msg_add_string(msg, SYSDB_MEMBEROF, group_dn_str);
            if (ret != LDB_SUCCESS) {
                ret = sysdb_error_to_errno(ret);
                goto done;
            }
            ...
            /* Extract username from the DN's first component */
            if (ldb_dn_get_comp_num(member_dn) > 0) {
                const struct ldb_val *v;
                v = ldb_dn_get_component_val(member_dn, 0);
                if (v && v->data) {
                    name_val = (const char *)v->data;
                    ret = ldb_msg_add_string(uid_msg, SYSDB_MEMBERUID,
                                             name_val);
                    if (ret != LDB_SUCCESS) {
                        ret = sysdb_error_to_errno(ret);
                        goto done;
        ...
        /* Emit one batched memberuid ADD for all new members */
        if (added_count > 0) {
            ret = sysdb_memberof_bypass_mod(domain, uid_msg);
            if (ret != EOK) {
                goto done;
            }
        }
